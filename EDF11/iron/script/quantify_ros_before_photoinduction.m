...
%% organize result
clear;load("..\data\quantification_ros.mat")
tbl = tbl_cell(ismember(tbl_cell.well,[13 34 35 29 24 10 11 1]),:);
tbl = sortrows(tbl,"erastinLv");

[G,ID] = findgroups(tbl.erastinLv);
...
for i = 1:length(ID)
    [f,xi] = ksdensity(tbl.mean_intensity(tbl.erastinLv==i));
    vCurve{i,1} = [f',xi'];
end
...
close all
figure
[p,~,stats] = anova1(tbl.mean_intensity,tbl.erastinLv,'off');
[c] = multcompare(stats);

PValues = nan(size(c,1),1);
for i = 1:length(PValues)
    x = tbl.mean_intensity(tbl.erastinLv==c(i,1));
    y = tbl.mean_intensity(tbl.erastinLv==c(i,2));
...
hold on
tblG = groupsummary(tbl.mean_intensity,tbl.erastinLv,{'mean' 'std'});
errorbar(1:8,tblG(:,1),tblG(:,2),'o','color','k','LineWidth',1,'MarkerFaceColor','k')
hold off

xlabel("Erastin (µM)")
ylabel("Effect size ("+effect+")")
ylabel('ROS (A.U.)')
set(gca,'Layer','top','LineWidth',1,'FontSize',16,'tickdir','out',...
    'XTick',1:8,'XTickLabel',string(fliplr([10 5 2.5 1.25 0.6 0.3 0.15 0])),'xlim',[0.5 8.5],'XColor','k')
box off
ylim([-4 64])
% exportgraphics(gcf,"..\fig\priming_ros.pdf","ContentType","vector")
