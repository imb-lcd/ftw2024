...
pos = "s"+setdiff((49:84),61); % well to process
...
for i = 1:length(pos)
    ...
    % load setting
    load("..\data\setting_"+prefix+"_"+pos(i)+".mat")

    if exist("..\data\kymograph_"+prefix+"_"+pos(i)+".mat",'file')
        isSave = 1;
    else
        isSave = 1;
    end
    ...
    % load images
    list = string(ls("..\img\"+prefix+"_"+pos(i)+"_*_ff1_10.tif"));
    list = list(contains(list,["cy5" "ros"])); % 2 signals are allowed
    img = cell(size(list));
    for j = 1:length(list)
    ...
    % speed measurement
%     th = [0.05 0.25]; % s2
%     th = [0.1 0.2]; % intensity
    th = [0.25 0.25]; % wavelength
%     th = [0.05 0.1]; % s11
    for j = 1:height(kymograph)
        maxR = kymograph.maxR{j};
        index_frame = (1:size(maxR,1))';
        ...
        frame_strt = index_frame(index(find(diff(index,2)==0,1)));
        frame_stop = index_frame(index(find(diff(index,2)==0,1,'last')+2));
        if kymograph.index_img(j)==2
            frame_stop = index_frame(ischange(test,"MaxNumChanges",1));
        end
        ...
        x_data = arrayfun(@(i)G.dist{i}(I{i}(1:min([nNbr length(G.dist{i})]))),1:length(I),'UniformOutput',0)';
        G.mode_dist = x_mode;
        G.data_dist = x_data;
...
%         G(find(diff(x_mode)<0)+1,:) = [];
%         G(find(ischange(x_mode,'MaxNumChanges',1))+1:end,:) = [];
    ...
    % save result
    if isSave
        exportgraphics(gcf,"..\img\check_speed_estimation\estimation_"+prefix+"_"+pos(i)+".jpg","Resolution",300)        
        save("..\data\kymograph_"+prefix+"_"+pos(i)+".mat","kymograph")
    end
